...
Rules and conventions for AI assistants and contributors working on corars.

## Overview

corars is a **multi-provider AI agent CLI** written in Rust. It connects to
...
protocol for host integration (e.g. Electron-based CoraUI).
...
|-------|-------|----------------|
| Bottom | `cora-types` | Shared provider-neutral data types (LLM, message, tool) — zero internal deps |
| Bottom | `cora-compact` | Context compression algorithms (folding, sanitization, tokenization) |
| Mid | `cora-config` | Configuration, ProviderCompat, auth, hooks, logging (`create_file_layer`), **cross-platform shell helpers** |
| Mid | `cora-protocol` | JSON stream protocol (events, commands, approval manager) for host integration |
| Mid | `cora-providers` | LLM provider implementations (Anthropic, OpenAI, Bedrock, Vertex) |
| Mid | `cora-tools` | Built-in agent tools (Read, Write, Edit, Bash, Grep, Glob, Spawn) |
| Mid | `cora-mcp` | MCP (Model Context Protocol) client |
| Mid | `cora-skills` | Skills system (prompt snippets, hooks, permissions, shell expansion) |
| Mid | `cora-memory` | Long-term cross-session memory (user prefs, feedback, project context) |
| Top | `cora-agent` | Agent engine, session management, orchestration |
| Top | `cora-cli` | CLI binary entry point |
...
- CoraCLI is the built-in SDK/library integration path. Do not model it as an
  external agent subprocess when changing Cora host integration code.
...
- All shell invocations must go through `cora_config::shell` module
...
| [json-stream-protocol.md](docs/json-stream-protocol.md) | JSON Lines protocol spec for host integration (e.g. CoraUI) |
