...
pub struct AgentEngine {
    ...
    msg_id: String,
    /// Turn id for the NEXT run, supplied by the host (e.g. CoraCore passes
...
impl AgentEngine {
    ...
    /// (e.g. CoraCore) pass them in so the session-side anchor matches their
    ...
    fn record_turn_usage(&mut self, turn_usage: &TokenUsage) -> bool {
        ...
        if context_usage == 0 {
            debug!(target: "cora_agent", "provider omitted turn usage; retaining local context projection");
            return false;
        }
