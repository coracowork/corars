...
const CORA_MARK_WIDTH: usize = 24;
// A mirror-symmetric line interpretation of the Cora mark using colons and spaces only.
const CORA_MARK_LINES: &[&str] = &[
    "          ::::          ",
    "        :::  :::        ",
    "      :::      :::      ",
    "    :::          :::    ",
...
fn welcome_transcript_lines(state: &AppState, width: u16) -> Vec<Line<'static>> {
    ...
    lines.push(
        Line::from(Span::styled("CoraCLI", normal(state).add_modifier(Modifier::BOLD))).alignment(Alignment::Center),
    );
    lines.push(
...
fn render_welcome(frame: &mut Frame<'_>, area: Rect, state: &AppState) {
    let welcome_height = CORA_MARK_LINES.len() as u16 + 3;
    ...
    lines.extend(cora_mark_lines(state));
    lines.push(Line::default());
...
fn cora_mark_lines(state: &AppState) -> Vec<Line<'static>> {
    CORA_MARK_LINES
        .iter()
        .map(|source| {
            debug_assert_eq!(source.len(), CORA_MARK_WIDTH);
            let spans = source
                .chars()
                .map(|glyph| {
