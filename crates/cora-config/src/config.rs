...
fn resolve_api_key(cli_key: Option<&str>, config_key: Option<&str>, provider: ProviderType) -> anyhow::Result<String> {
    ...
    anyhow::bail!(
        "No API key found. Provide via --api-key, config file, environment variable \
         (API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY), or run 'corars auth login'."
    )
}
...
///
/// - Linux:   `~/.config/corars`
/// - macOS:   `~/Library/Application Support/corars`
/// - Windows: `%APPDATA%\corars`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("corars"))
}
...
const DEFAULT_CONFIG_TEMPLATE: &str = r#"# corars configuration
...
# OAuth settings (for `corars auth login` with Claude.ai account)
# [auth]
...
[session]
enabled = true
directory = ".corars/sessions"  # relative to project root
...
# dir = "~/Library/Logs/corars"    # log directory (default: platform-specific)
