...
2. Substitutes variables (`$ARGUMENTS`, `$0`, `${CORARS_SKILL_DIR}`)
...
|----------|------|-------------|
| 1 | `.corars/skills/` | Project-local skills (checked-in with the repo) |
| 2 | `<CONFIG_DIR>/corars/skills/` | User-global skills (see below) |
| 3 | `.corars/commands/` | Legacy flat `.md` files (backward compatibility) |
...
>
> Run `corars skills path` to see the actual paths on your machine.
...
| `${CORARS_SKILL_DIR}` | Absolute path to the directory containing this skill's `SKILL.md` |
...
Working directory: ${CORARS_SKILL_DIR}
```
...
Use `corars skills path` to see which directories are being scanned and whether
they exist on disk:

```
$ corars skills path
User:    ~/Library/Application Support/corars/skills  (exists)
Project: /path/to/repo/.corars/skills                 (exists)
Legacy:  /path/to/repo/.corars/commands                (not found)
```
