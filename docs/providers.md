...
```bash
corars --profile claude-fast "Quick question"
corars --profile claude-deep "Deep security audit"
corars --profile dev "Create a GitHub issue"
```
...
3. Credentials are saved alongside the global config (run `corars config path` to find the directory)
