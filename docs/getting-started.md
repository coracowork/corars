...
|------------|--------------|
| `corars config init` | Generate a default global config file |
| `corars config path` | Print the global config file path |
| `corars auth login` | Login with Anthropic account (OAuth device flow) |
| `corars auth logout` | Logout (remove saved OAuth credentials) |
| `corars session list` | List saved sessions |
| `corars skills path` | Print skill directory paths |
...
| `--log-level <filter>` | Log level filter (e.g. `debug`, `info`, `cora_providers=debug`) |
...
```
<global config>                   (global, user-level; run `corars config path` to find)
    ↓ overridden by
./.corars.toml                  (project-level, working directory)
    ↓ overridden by
...
```bash
corars config init
# Creates the global config file (run `corars config path` to see the location)
```

### Config File Format

```toml
# Global config file (path varies by OS, use `corars config path` to find)
...
5. OAuth credentials (via `corars auth login`)
...
```bash
corars config init
# Edit the config file (run `corars config path` to find it), add your API key
```

### 2. Single-Shot Mode

```bash
corars "Read and explain crates/cora-agent/src/engine.rs"
```
...
```bash
corars --profile deepseek "Fix the bug in main.rs"
corars --profile ollama "Analyze code quality"
```
...
corars "List all Rust files in this project"
```
...
Sessions auto-save to `.corars/sessions/`.
...
corars --resume latest

# Resume a specific session
corars --resume a1b2c3

# Create a session with a custom ID
corars --session-id my-conv-123
```
