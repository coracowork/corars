...
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .corars/audit.log"
...
```bash
# CLI — logs to /tmp/corars-logs at debug level
corars --log-dir /tmp/corars-logs --log-level debug "Read Cargo.toml"
```
...
| `"cora_providers=debug,info"` | Debug for providers, info for everything else |
...
| Platform | Path |
|----------|------|
| macOS | `~/Library/Logs/corars/` |
| Linux | `$XDG_STATE_HOME/corars/logs/` or `~/.local/state/corars/logs/` |
| Windows | `{data_local_dir}/corars/logs/` |
...
```json
{"timestamp":"2026-05-13T12:12:52.431Z","level":"INFO","fields":{"message":"mcp server connected","server":"sentry","tools":20},"target":"cora_mcp","spans":[{"name":"agent_run","session_id":"abc-123","msg_id":"msg-456"}]}
```
...
| Field | Description |
|-------|-------------|
| `target` | Source crate (`cora_agent`, `cora_providers`, `cora_mcp`, etc.) |
...
grep '"session_id":"abc-123"' 2026-05-13.corars.log | jq .
```

### Library Integration

When corars is used as a library (e.g. embedded in a backend server), the `create_file_layer()` API provides a composable tracing layer:
...
let resolved = ResolvedLogging {
    enabled: true,
    level: "cora_agent=debug,cora_providers=debug".to_string(),
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // corars logs → separate corars.log file
    .init();
```

The host application owns the global subscriber; corars library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/corars/AGENTS.md` — user-level instructions for all projects
...
```
<config_dir>/corars/projects/<sanitized-project-path>/memory/
...
```bash
export CORARS_MEMORY_DIR=/custom/path
```
...
plan_directory = ".corars/plans"  # Where plan files are saved
```
